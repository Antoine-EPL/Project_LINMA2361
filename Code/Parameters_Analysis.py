...
# Function to plot energy evolution for varying parameters
def make_plot_energy_varying_parameter(N, alpha, beta, K, M, T, h, param_name, param_values):
    adjacency_psi = adjacency_matrix("1", N)
    adjacency_phi = adjacency_matrix("3", N)
    Z = pattern_formation('circle', N)
    ...
    for param_value in param_values:
        ...
        # Compute energies
        kinetic_energy = compute_kinetic_energy(V)
        potential_energy = compute_potential_energy(X, adjacency_phi, beta, Z, M)
        ...
        # Plot total energy
        ax.plot(time, total_energy, label=f"{param_name}={param_value}")

    # Set plot title and labels
    ax.set_title(f"Evolution of total energy as a function of the parameter {param_name}", fontsize=12)
    ...
    plt.savefig(f"Analysis_parameters_{param_name}.pdf", dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    param_name = "alpha"  # Choose from "alpha", "beta", "K", or "M"
    param_values = [1, 1.3, 1.6, 1.8, 1.905]  # Example values for the chosen parameter

    # Call the function to plot
    make_plot_energy_varying_parameter(N, alpha, beta, K, M, T, h, param_name, param_values)

    # Parameter to vary and its values
    param_name = "beta"  # Choose from "alpha", "beta", "K", or "M"
    param_values = [0.2, 0.4, 0.6, 0.8, 0.9]  # Example values for the chosen parameter

    # Call the function to plot
    make_plot_energy_varying_parameter(N, alpha, beta, K, M, T, h, param_name, param_values)

    # Parameter to vary and its values
    param_name = "K"  # Choose from "alpha", "beta", "K", or "M"
    param_values = [1, 5, 7.5, 15, 30]  # Example values for the chosen parameter

    # Call the function to plot
    make_plot_energy_varying_parameter(N, alpha, beta, K, M, T, h, param_name, param_values)

    # Parameter to vary and its values
    param_name = "M"  # Choose from "alpha", "beta", "K", or "M"
    param_values = [2, 25, 75, 100, 200]  # Example values for the chosen parameter
    ...
    makeplot_animation_only_agents(X,V,N,T,h,Z,SAVEFIG=True,filename_template="animation_only_agents_G_psi_1_G_phi_3_alpha_1_1_beta_0_5_K_15_M_25")
