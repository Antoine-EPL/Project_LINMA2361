...
def make_plot_energy_comparison_modif_phi(basis_graph_type,N, alpha, beta, K, M, T, h):
    adjacency_psi = adjacency_matrix(basis_graph_type, N)
    Z = pattern_formation('circle', N)

    fig, ax = plt.subplots(figsize=(9, 6))

    for i in range(4):  # adjacency_phi type from 0 to 3
        adjacency_phi = adjacency_matrix(str(i), N)
        ...
        # Plot total energy
        ax.plot(time, total_energy, label=f'G{i}')

    ax.set_title('Energy evolution for different graph configurations for control term', fontsize=12)
    ...
    plt.savefig(f"Energy_comparison_for_differents_graphs_for_phi_{basis_graph_type}.pdf", dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    """
    for i in range(4): 
        make_plot_energy_comparison_modif_phi(str(i),N,alpha,beta,K,M,T,h)
    """
    ...
    makeplot_animation_only_agents(X,V,N,T,h,Z,SAVEFIG=True,filename_template="animation_only_agents_G_psi_0_G_phi_1")
    ...
    makeplot_animation_only_agents(X,V,N,T,h,Z,SAVEFIG=True,filename_template="animation_only_agents_G_psi_1_G_phi_3")
